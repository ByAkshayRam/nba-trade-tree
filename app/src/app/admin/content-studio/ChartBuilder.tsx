                ...
                {loading ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <ScatterChart margin={{ top: 10, right: 40, bottom: 30, left: 10 }}>
                      ...
                      <XAxis dataKey="x" type="number" name="Home-Grown" tick={{ fill: theme.axis, fontSize: 11 }} label={{ value: "Home-Grown Players", position: "bottom", fill: "#888", fontSize: 12 }} />
                      <YAxis dataKey="y" type="number" name="Wins" tick={{ fill: theme.axis, fontSize: 11 }} label={{ value: "Wins", angle: -90, position: "insideLeft", fill: "#888", fontSize: 12 }} />
                      <ZAxis range={[300, 300]} />
                      <Tooltip content={({ payload }) => {
                        if (!payload?.[0]) return null;
                        const d = payload[0].payload;
                        return <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 6, padding: "8px 12px", color: "#fff", fontSize: 12 }}><strong>{d.abbr}</strong>: {d.x} home-grown, {d.y} wins</div>;
                      }} />
                      <ReferenceLine
                        segment={[
                        ...
                        stroke={darkMode ? "#22d3ee" : "#0891b2"} strokeDasharray="6 4" strokeWidth={2} opacity={0.5}
                      />
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={sortedTeams} margin={{ top: 20, right: 20, bottom: 20, left: 10 }}>
                      ...
                      <XAxis
                        ...
                        height={40}
                      />
                      <YAxis tick={{ fill: theme.axis, fontSize: 11 }} axisLine={false} tickLine={false} domain={[0, 100] as const} ticks={[0, 25, 50, 75, 100]} tickFormatter={(v: number) => `${Math.round(v)}%`} allowDataOverflow />
                      ...
                      <Legend
                        ...
                        iconType="circle"
                        onClick={(e: any) => { setActiveSegment(prev => prev === e.dataKey ? null : e.dataKey); }}
                        formatter={(value: string, entry: any) => <span style={{ color: !activeSegment || activeSegment === entry.dataKey ? "#999" : "#444" }}>{value}</span>}
                      />
                      {preset === "acquisition" ? (
                        <>
